...
```
src/
  lib.rs                 Library root (re-exports config, motor, arm)
  config.rs              Typed config loader (serde_yaml)
...
tests/
  motor_test.rs          Config + hardware integration tests
```
...
- Windows with CH340 driver installed
- `cargo build` handles all crate dependencies automatically
